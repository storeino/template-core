...
export default async function ({ $http ,route, $tools, $storeino, store, app }, inject) {
  if(process.server) {
    let response = null;
    ...
    else if(settings.store_currencies.find(c=>c.code == store.state.currency.code)) {
      store.state.currency.symbol = settings.store_currencies.find(c=>c.code == store.state.currency.code).symbol;
    } 
